...
(function ($) {
    $.fn.slideform = function (userOptions, args) {
        ...
        function init() {
            ...
            // Adding the track.
            $track = $("<div />", { class: "slideform-track" }).insertBefore($form.find(".slideform-slide").eq(0));

            // Adding the wrapper.
            $wrapper = $("<div />", { class: "slideform-wrapper" }).appendTo($track).append($form.find(".slideform-slide"));
            ...
            $form.find(".slideform-slide").each(function (k, v) {
                ...
                if (k < $slides.length - 2) {
                    $(this)
                        ...
                        .append('<button class="slideform-btn slideform-btn-next">' + $options.nextButtonText + "</button>");
        ...
        function triggerConditions() {
            ...
            $form.serializeArray().map(function (i) {
                if (typeof input[i.name] === "undefined") {
                    input[i.name] = i.value;
                } else if (Array.isArray(input[i.name])) {
                    input[i.name].push(i.value);
                } else {
                    input[i.name] = [input[i.name], i.value];
                }
            });
            ...
            $form.find("[data-condition]").each(function () {
                ...
                if (valid) {
                    ...
                    $(this).slideUp();
                    eval($(this).data("condition-false"));
                }
            });
        }
